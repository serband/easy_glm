...
Replays thirteen workbench failure modes (four ways to lose work and nine
tracebacks) using Streamlit's test harness on a small synthetic motor book, and
writes a plain-language page: what the tester did, what used to happen and what
the tool says now.

Usage: python scripts/checks/w3_hardening.py [--write]
  --write regenerates .internal/checks/w3-hardening.md; otherwise the page is printed.
"""
...
DOC = ROOT / ".internal" / "checks" / "w3-hardening.md"
N = 3000
...
"""
    ...
    lines = [
        ...
        "(text policy numbers, a 0/1 train flag, three rating factors). Rows 1–4 "
        'exercise lost-work protections and rows 5–13 exercise former tracebacks. "Now" '
        "is what this run of the tool actually said.*",
        "",
    ...
    lines += [
        ...
        "propagate correctly) and 35 (the 'pick two different variables' message shows "
        "in every Diagnostics tab) are cosmetic and left for a later piece. Item 32 is "
        "fixed: the Seed box now always shows the seed the project holds, so the page "
        "can no longer name a seed the split did not come from. Items 28 (a fit "
        "interrupted by a page refresh) and 38 (a constant column blocking the fit) "
        "are covered by `tests/test_w4_runs_folder.py`, together with persisted-run "
