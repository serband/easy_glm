...
df = pl.DataFrame(
    {
        ...
        "Region": rng.choice(
            ["North", "South", "Urban", "Rural"], n, p=[0.35, 0.35, 0.20, 0.10]
        ),
...
if len(ref_indices) > 0:
    ...
    assert (ref_row == 0).all(), "Reference level should have all zeros"
...
for level in blueprint["Region"][1:]:
    ...
    if len(indices) > 0:
        ...
        if expected_col in row.index:
            ...
            assert (row[others] == 0).all(), "Other columns should be 0"
