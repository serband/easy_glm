...
test('interaction-only refits visibly reuse mains; main settings changes refit', async ({
    page,
}, testInfo) => {
    ...
    await button('Fit model').click();
    await expect(
        page.getByText('Unchanged main effects reused. Diagnostics and rate tables are ready.', {
            exact: true,
        }),
    ).toBeVisible({ timeout: 45000 });
    await page.screenshot({
        path: testInfo.outputPath('easyglm-main-effects-reuse.png'),
        fullPage: true,
    });
