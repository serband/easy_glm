...
# ── Score new data with the exported model ──────────────────────────
loaded = RateModel.from_json("french_motor.easyglm")

sample = raw.head(3)
preds = loaded.predict(sample)
print(f"Predictions (with exposure): {preds}")

preds_no_exp = loaded.predict(sample, exposure_col=None)
print(f"Predictions (no exposure):   {preds_no_exp}")

sample_renamed = sample.rename({"Exposure": "Exp"})
preds_override = loaded.predict(sample_renamed, exposure_col="Exp")
print(f"Predictions (renamed exp):   {preds_override}")

# ── Score full datasets & plot Actual vs Expected ───────────────────
...
print(f"\nOverall Test A/E: {test_data['ClaimNb'].sum() / test_preds.sum():.4f}")
...
fig.legend(["Actual", "Expected"], loc="lower center", ncol=2, fontsize=10)
