...
def _two_stage_code(
    ...
    if cfg.offset:
        eta1 += f" + train[{cfg.offset!r}].to_numpy()"
    return [
...
def to_script(
    ...
    *,
    ...
    # a fitted run is the authority on whether the design really has cells (a
    # parent that could not be encoded takes its interaction with it)
    two_stage = bool(run.spec.interactions if run is not None else cfg.interactions)
    ...
    if run is not None:
        ...
        alpha2 = stage2_alpha(cfg) if two_stage else None
        if two_stage and alpha2 is None:
            alpha2 = alpha
        chose_by_cv = False
        monotone = {
            **{
