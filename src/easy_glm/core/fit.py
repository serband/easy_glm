...
def fit_glm(
    ...
    **glum_kwargs: Any,
    ...
    # per-row exposure per rate-table row, from the ORIGINAL rows (aggregation
    # below replaces sw with grouped weights)
    ...
    **on top** of that fit — no intercept, and stage 1's linear predictor (plus
    ...
    def __init__(self, stage1: GLMFit, stage2: GLMFit) -> None:
        ...
        super().__init__(
            ...
            # the mains' training exposure per rate-table row comes from stage 1
            # like everything else about the mains; without it the rate tables of
            # a two-stage model would carry no exposure and the relativity tools
            # would weigh every band the same
            row_exposure=dict(stage1.row_exposure),
